...
class WPLM_List_Licenses extends WP_List_Table {
	...
	function column_product_ref( $item ) {
		$project_id = intval( $item['product_ref'] );
		if ( $project_id > 0 ) {
			$project_title = get_the_title( $project_id );
			$edit_link = get_edit_post_link( $project_id );
			if ( $project_title && $edit_link ) {
				return sprintf( '<a href="%s"><strong>%s</strong></a>', esc_url( $edit_link ), esc_html( $project_title ) );
			} elseif ( $project_title ) {
				return esc_html( $project_title );
			}
		}
		return esc_html( $item['product_ref'] );
	}
